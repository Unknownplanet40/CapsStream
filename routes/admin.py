...
def api_scan():
    ...
    if pid and not is_admin():
        try:
            ...
            from routes.middleware import ACTIVE_PROFILE_SESSIONS, ACTIVE_PROFILE_LOCK
        with ACTIVE_PROFILE_LOCK:
...
def api_system_backup_status():
    BASE_DIR = current_app.config.get("BASE_DIR") or os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
